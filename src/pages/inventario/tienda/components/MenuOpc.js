...
const MenuOpcTd = ({ children }) => {
  ...
  useEffect(() => {
    getMethod({
      path: `/inventario/consultar-opciones`,
      setData,
      setIsLoading: () => {},
      showSwal: false,
    });
  ...
  const goToProduct = (item) => {
    ...
    navigate(`/inventario-producto/${item.id_producto}/${item.nombre}`);
  };
  ...
  const goToSubProduct = (opc, item) => {
    ...
    navigate(
      `/inventario-producto/${opc.id_producto}/${item.nombre}/${opc.id_subproducto}/${opc.nombre}`
    );
  };
  ...
  return (
    <>
      <div
        className="bg-[#88e3d5] py-2 min-h-14 w-full"
            ...
            data.map((item) => {
              return (
                <Dropdown
                  ...
                  onClick={() => {
                    if (item.id_producto === 0) navigate(`/productos`);
                  }}
